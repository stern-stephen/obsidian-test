...
A **projection matrix** sends vectors onto a subspace.

If $P$ is a projection matrix, then applying it twice has the same effect as applying it once:
...
For real matrices, this becomes:
...
Projection matrices have simple [Eigenvalues](Eigenvalues.md).
...
A projection matrix separates a vector into two pieces:
