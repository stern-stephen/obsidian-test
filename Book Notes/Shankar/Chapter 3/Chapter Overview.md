# Chapter Overview

Source: [Principles of Quantum Mechanics](../../../References/Principals%20of%20quantum%20mechanics%20Shankar%20.pdf)

Book hub: [Shankar](../Shankar.md)

Book chapter: 3, pages 107-113.

Previous: [Symmetries and Their Consequences](../Chapter%202/Symmetries%20and%20Their%20Consequences.md)

Next: [Particles and Waves in Classical Physics](Particles%20and%20Waves%20in%20Classical%20Physics.md)

## Reading Status

- Status: started
- Pages: 107-113
- Date started: 2026-05-31
- Date finished:

## Big Ideas

- Chapter 3 is Shankar's short physical motivation for quantum mechanics.
- Classical physics separates particles and waves cleanly, but microscopic experiments do not respect that separation.
- The double-slit experiment shows why definite classical trajectories cannot be the whole story.
- Light and matter both force a mixed lesson: localized detections occur, but probabilities are governed by interference.
- The chapter prepares the meaning of a quantum state as a wavefunction or ket whose squared amplitude predicts detection probabilities.

## Section Notes

- [Particles and Waves in Classical Physics](Particles%20and%20Waves%20in%20Classical%20Physics.md)
- [An Experiment with Waves and Particles](An%20Experiment%20with%20Waves%20and%20Particles.md)
- [The Double-Slit Experiment with Light](The%20Double-Slit%20Experiment%20with%20Light.md)
- [Matter Waves](Matter%20Waves.md)
- [Conclusions](Conclusions.md)

## Links To Concept Notes

- [Quantum Mechanics](../../../Quantum%20Mechanics/Quantum%20Mechanics.md)
- [Wave-Particle Duality](../../../Quantum%20Mechanics/Wave-Particle%20Duality.md)
- [Quantum State Vector](../../../Quantum%20Mechanics/Quantum%20State%20Vector.md)
- [Quantum Measurement](../../../Quantum%20Mechanics/Quantum%20Measurement.md)
- [Postulates of Quantum Mechanics](../../../Quantum%20Mechanics/Postulates%20of%20Quantum%20Mechanics.md)

## Notes To Process Later

- Add diagrams or image references for the two-slit setup if they become useful for memory.
- Revisit this chapter after Chapter 5, where Shankar returns to the double-slit experiment with the one-dimensional formalism in place.

<!-- semantic-edges
{"source":"Chapter Overview","relation":"PART_OF","target":"Shankar Chapter 3","evidence_heading":"Book metadata","evidence_summary":"This note is a source-specific section note in Shankar Chapter 3.","confidence":0.85}
{"source":"Chapter Overview","relation":"SOURCE_CONTEXT_FOR","target":"Symmetries and Their Consequences","evidence_heading":"Chapter Overview","evidence_summary":"This source note explicitly links its treatment to Symmetries and Their Consequences.","confidence":0.8}
{"source":"Chapter Overview","relation":"SOURCE_CONTEXT_FOR","target":"Particles and Waves in Classical Physics","evidence_heading":"Chapter Overview","evidence_summary":"This source note explicitly links its treatment to Particles and Waves in Classical Physics.","confidence":0.8}
{"source":"Chapter Overview","relation":"SOURCE_CONTEXT_FOR","target":"An Experiment with Waves and Particles","evidence_heading":"Section Notes","evidence_summary":"This source note explicitly links its treatment to An Experiment with Waves and Particles.","confidence":0.8}
{"source":"Chapter Overview","relation":"SOURCE_CONTEXT_FOR","target":"The Double-Slit Experiment with Light","evidence_heading":"Section Notes","evidence_summary":"This source note explicitly links its treatment to The Double-Slit Experiment with Light.","confidence":0.8}
-->
