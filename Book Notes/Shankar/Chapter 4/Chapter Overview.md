# Chapter Overview

Source: [Principles of Quantum Mechanics](../../../References/Principals%20of%20quantum%20mechanics%20Shankar%20.pdf)

Book hub: [Shankar](../Shankar.md)

Book chapter: 4, pages 115-150.

Previous: [Conclusions](../Chapter%203/Conclusions.md)

Next: [The Postulates](The%20Postulates.md)

## Reading Status

- Status: started
- Pages: 115-150
- Date started: 2026-05-31
- Date finished:

## Big Ideas

- Chapter 4 states the postulates of nonrelativistic quantum mechanics for one particle in one dimension.
- The state is a Hilbert-space vector, not a point in classical phase space.
- Observables are Hermitian operators, and measurement outcomes are eigenvalues.
- Measurement is probabilistic and generally changes the state.
- Time evolution is governed by the Schrodinger equation with a Hamiltonian operator.
- The chapter translates the physical motivation from the double-slit experiment into a working mathematical rulebook.

## Section Notes

- [The Postulates](The%20Postulates.md)
- [Discussion of Postulates I-III](Discussion%20of%20Postulates%20I-III.md)
- [The Schrodinger Equation](The%20Schrodinger%20Equation.md)

## Links To Concept Notes

- [Postulates of Quantum Mechanics](../../../Quantum%20Mechanics/Postulates%20of%20Quantum%20Mechanics.md)
- [Quantum State Vector](../../../Quantum%20Mechanics/Quantum%20State%20Vector.md)
- [Quantum Measurement](../../../Quantum%20Mechanics/Quantum%20Measurement.md)
- [Schrodinger Equation](../../../Quantum%20Mechanics/Schrodinger%20Equation.md)
- [Hermitian Matrices and Operators](../../../Linear%20Algebra/Hermitian%20Matrices%20and%20Operators.md)
- [Commutators](../../../Linear%20Algebra/Commutators.md)

## Notes To Process Later

- Add more worked examples from the fictitious finite-dimensional Hilbert spaces after doing the exercises.
- Revisit operator ordering once later chapters give more examples beyond $X$ and $P$.
