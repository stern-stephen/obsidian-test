# The Electromagnetic Force in the Hamiltonian Scheme

Source: [Principles of Quantum Mechanics](../../../References/Principals%20of%20quantum%20mechanics%20Shankar%20.pdf)

Book hub: [Shankar](../Shankar.md)

Book section: 2.6.

Previous: [The Hamiltonian Formalism](The%20Hamiltonian%20Formalism.md)

Next: [Cyclic Coordinates, Poisson Brackets, and Canonical Transformations](Cyclic%20Coordinates%20Poisson%20Brackets%20and%20Canonical%20Transformations.md)

## Reading Status

- Status: started
- Pages:
- Date started: 2026-05-19
- Date finished:

## Big Ideas

- Electromagnetic coupling has a clean Hamiltonian form when canonical and mechanical momentum are kept distinct.
- The combination $\mathbf{p} - q\mathbf{A}$ is the mechanical momentum.
- The same substitution becomes minimal coupling in quantum mechanics.

## Notes

For a charged particle in electromagnetic potentials, the Hamiltonian can be written:

$$
H = \frac{1}{2m}(\mathbf{p} - q\mathbf{A})^2 + q\phi
$$

Here $\mathbf{p}$ is canonical momentum. The velocity is related to:

$$
m\dot{\mathbf{r}} = \mathbf{p} - q\mathbf{A}
$$

This is the Hamiltonian version of the same physics encoded in the electromagnetic Lagrangian.

## Quantum Bridge

The classical expression suggests the quantum replacement:

$$
\mathbf{p} \to -i\hbar\nabla
$$

with electromagnetic coupling introduced through:

$$
-i\hbar\nabla \to -i\hbar\nabla - q\mathbf{A}
$$

## Common Confusions

- The canonical momentum is the phase-space coordinate; the mechanical momentum is tied directly to velocity.
- The vector potential appears in the Hamiltonian even though the magnetic force depends on velocity crossed with magnetic field.
- Minimal coupling is not an arbitrary quantum trick; it is already visible in the classical Hamiltonian.

## Links To Concept Notes

- [Electromagnetic Lagrangian](../../../Mechanics/Electromagnetic%20Lagrangian.md)
- [Canonical Momentum](../../../Mechanics/Canonical%20Momentum.md)
- [Hamiltonian Mechanics](../../../Mechanics/Hamiltonian%20Mechanics.md)

<!-- semantic-edges
{"source":"Shankar Section 2.6","relation":"EXAMPLE_OF","target":"Hamiltonian Electromagnetic Coupling","evidence_heading":"Big Ideas","evidence_summary":"Uses electromagnetic coupling to show why canonical and mechanical momentum must remain distinct in Hamiltonian mechanics.","confidence":0.9}
{"source":"Shankar Section 2.6","relation":"MOTIVATES","target":"Canonical-Mechanical Momentum Distinction","evidence_heading":"Common Confusions","evidence_summary":"The canonical momentum is the phase-space coordinate, while mechanical momentum is directly tied to velocity.","confidence":0.9}
{"source":"Classical Minimal Coupling","relation":"MOTIVATES","target":"Quantum Minimal Coupling","evidence_heading":"Quantum Bridge","evidence_summary":"Shows the classical substitution that becomes the quantum replacement of the momentum operator by an electromagnetically coupled operator.","confidence":0.89}
-->
