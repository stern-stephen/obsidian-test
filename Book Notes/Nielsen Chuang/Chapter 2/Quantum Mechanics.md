# Quantum Mechanics

Source: [quantum-computation-and-quantum-information-nielsen-chuang](../../../References/quantum-computation-and-quantum-information-nielsen-chuang.pdf)

Book hub: [Nielsen And Chuang](../Nielsen%20Chuang.md)

## Reading Status

- Status: Notes in progress
- Pages: 60-119
- Date started:
- Date finished:

## Big Ideas

- Section 2.1 reviews the linear algebra used throughout quantum mechanics. See [Linear Algebra](Linear%20Algebra.md).
- Section 2.2 states the core postulates: state spaces, unitary evolution, measurement, phase, and composite systems.
- Sections 2.3 through 2.6 show the postulates in action through superdense coding, density operators, Schmidt decomposition, purification, EPR, and Bell inequalities.

## Section Notes

- [Linear Algebra](Linear%20Algebra.md)
- [Postulates Of Quantum Mechanics](Postulates%20of%20Quantum%20Mechanics.md)
- [Superdense Coding](Superdense%20Coding.md)
- [Density Operator](Density%20Operator.md)
- [Schmidt Decomposition And Purifications](Schmidt%20Decomposition%20and%20Purifications.md)
- [EPR And The Bell Inequality](EPR%20and%20Bell%20Inequality.md)

## Definitions

- Quantum state
- Unitary evolution
- Measurement operator
- Projective measurement
- POVM
- Density operator
- Reduced density operator
- Schmidt decomposition
- Purification
- Bell inequality

## Equations

$$
|\psi'\rangle = U|\psi\rangle
$$

## Questions

- Why are measurement probabilities quadratic in amplitudes?
- How does entanglement make a subsystem look mixed?
- What precisely separates Bell inequality violation from faster-than-light signalling?

## Links To Concept Notes

- [Quantum Mechanics](../../../Quantum%20Mechanics/Quantum%20Mechanics.md)
- [Linear Algebra](Linear%20Algebra.md)
- [Postulates Of Quantum Mechanics](Postulates%20of%20Quantum%20Mechanics.md)
- [Density Operator](Density%20Operator.md)
- [Bra-Ket Notation](../../../Linear%20Algebra/Bra-Ket%20Notation.md)
- [Vector Spaces](../../../Linear%20Algebra/Vector%20Spaces.md)
- [Unitary Matrices](../../../Linear%20Algebra/Unitary%20Matrices.md)
- [Hermitian Matrices and Operators](../../../Linear%20Algebra/Hermitian%20Matrices%20and%20Operators.md)

## Notes To Process Later

- Create durable concept notes for tensor products, density operators, entanglement, and measurement.
